...
describe('User Service', () => {
  describe('getUserById', () => {
    it('should return a user if found', async () => {
      const mockUser = { _id: '123', username: 'testuser', email: 'testuser@example.com' };
  ...
  describe('updateUser', () => {
    it('should update and return the user if found', async () => {
      const mockUser = { _id: '123', username: 'updateduser', email: 'updateduser@example.com' };
      (User.findByIdAndUpdate as jest.Mock).mockResolvedValue(mockUser);

      const result = await updateUser('123', { username: 'updateduser' });

      expect(User.findByIdAndUpdate).toHaveBeenCalledWith('123', { username: 'updateduser' }, { new: true });
      expect(result).toEqual(mockUser);
    });
    ...
    it('should throw an error if user not found', async () => {
      ...
      await expect(updateUser('123', { username: 'updateduser' })).rejects.toThrow('User not found');
    });
  });
