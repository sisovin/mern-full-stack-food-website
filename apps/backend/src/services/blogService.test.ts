import { getAllBlogPosts, getBlogPostById, createBlogPost, updateBlogPost, deleteBlogPost } from './blogService';
...
describe('Blog Service', () => {
  ...
  describe('getBlogPostById', () => {
    it('should return a blog post by ID', async () => {
      const mockBlogPost = { _id: '1', title: 'Blog 1', content: 'Content 1', author: 'Author 1' };
  ...
  describe('createBlogPost', () => {
    it('should create a new blog post', async () => {
      const mockBlogPostData = { title: 'Blog 1', content: 'Content 1', author: 'Author 1' };
  ...
  describe('updateBlogPost', () => {
    it('should update a blog post', async () => {
      const mockBlogPostData = { title: 'Updated Blog 1', content: 'Updated Content 1' };
      const mockBlogPost = { _id: '1', ...mockBlogPostData };
      (Blog.findByIdAndUpdate as jest.Mock).mockResolvedValue(mockBlogPost);

      const result = await updateBlogPost('1', mockBlogPostData);

      expect(Blog.findByIdAndUpdate).toHaveBeenCalledWith('1', mockBlogPostData, { new: true });
      expect(result).toEqual(mockBlogPost);
    });
    ...
    it('should throw an error if blog post not found', async () => {
      ...
      await expect(updateBlogPost('1', {})).rejects.toThrow('Blog post not found');
    });
  });
